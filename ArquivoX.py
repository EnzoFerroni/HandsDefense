...
class Sinal(pygame.sprite.Sprite):
    def __init__(self, num):
        super(Sinal, self, position).__init__()
        self.position = pygame.math.Vector2((position[0], position[1]))
        self.image = pygame.image.load("./imagens/sinais/" + num + ".png")
        self.rect = self.image.get_rect()
        self.rect.center = self.position
    def update():
        self.rect.center = self.position
...
class Enemy1(pygame.sprite.Sprite):
    def __init__(self):
        ...
        fat = 0
        for e in listaTemp:
            self.sinal_image.append([pygame.image.load("./Imagens/libras/" + e + ".png"), [self.position.x + fat, self.position.y - 50]])
            fat += 11
